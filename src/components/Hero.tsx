...
export default function Hero() {
  ...
  /* ── Parallax: update --scroll-y on the hero element ── */
  useEffect(() => {
    let raf = 0
    const onScroll = () => {
  ...
  return (
    <section ref={heroRef} className="hero-section" style={{ '--scroll-y': '0' } as React.CSSProperties}>

      {/* ── Background: stacked CSS gradients, zero extra DOM ── */}
      {/* Parallax orb moves via an absolutely positioned layer that only carries the transform */}
      <div style={{
        position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none',
        background: `
          radial-gradient(circle at 90% 15%, rgba(70,132,50,0.28)  0%, transparent 45%),
          radial-gradient(circle at 5%  80%, rgba(255,160,46,0.13)  0%, transparent 40%),
          radial-gradient(circle, rgba(154,216,114,0.08) 1px, transparent 1px)
        `,
      ...
      <div className="container" style={{ display: 'contents' }}>
        ...
        {/* ── LEFT ── */}
        <div className="hero-left" style={{ paddingLeft: 40, position: 'relative', zIndex: 2 }}>

          {/* Eyebrow */}
          <div className="reveal" style={{ marginBottom: 22 }}>
            <span className="eyebrow">POS · Inventory · Accounting</span>
          </div>
          ...
          <p className="reveal reveal-2" style={{
            ...
            maxWidth: 400, lineHeight: 1.75, marginBottom: 36,
          }}>
            All-in-one platform for African businesses — POS, inventory
            and accounting that works fully offline.
          </p>

          <div className="reveal reveal-3" style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 44 }}>
            <a href="#pricing" className="btn btn-primary" style={{ padding: '14px 28px', fontSize: 15, borderRadius: 10 }}>
              Start free — 14 days
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
            </a>
            <a href="#features" className="btn" style={{
              background: 'rgba(255,255,255,0.07)', color: '#fff',
              border: '1.5px solid rgba(255,255,255,0.14)', padding: '14px 22px', fontSize: 15, borderRadius: 10,
            }}>
              See how it works
            </a>
          </div>
          ...
          {/* Stats pills */}
          <div className="reveal reveal-3" style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {[
              { v: '500+', l: 'Active shops',  color: 'var(--orange)' },
              { v: '4.9★', l: 'Avg rating',    color: '#9AD872' },
              { v: '100%', l: 'Offline ready', color: '#FFEF91' },
            ].map((s, i) => (
              <div key={i} style={{
                ...
                <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, fontSize: 18, color: s.color, letterSpacing: '-0.02em', lineHeight: 1 }}>{s.v}</span>
                <span style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.48)', marginTop: 4 }}>{s.l}</span>
              </div>
            ))}
          </div>
        </div>

        {/* ── RIGHT — layered devices ── */}
        <div className="hero-right" style={{ position: 'relative', zIndex: 2 }}>
          <div style={{ position: 'relative', width: '100%', maxWidth: 580, paddingBottom: 80 }}>

            {/* Midground glow behind devices */}
            <div style={{
              position: 'absolute', top: '10%', left: '5%', right: '5%', bottom: '10%',
              borderRadius: 24,
              background: 'radial-gradient(ellipse at 50% 40%, rgba(70,132,50,0.15) 0%, transparent 70%)',
              filter: 'blur(20px)',
            ...
            {/* ── Laptop (foreground, moves fastest) ── */}
            ...
            {/* ── Phone — bottom right, separate parallax layer ── */}
            ...
            {/* ── Floating chip — payment notification ── */}
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: '#888', marginTop: 1 }}>TZS 28,703 · Transfer</div>
              </div>
            </div>
            ...
            <div style={{
              ...
              background: 'rgba(13,15,20,0.92)', borderRadius: 12, padding: '9px 13px',
              ...
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#9AD872" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 4v6h-6"/><path d="M1 20v-6h6"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg>
              <div>
                <div style={{ fontSize: 10, fontWeight: 600, color: '#9AD872', whiteSpace: 'nowrap' }}>All branches synced</div>
                <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.4)', marginTop: 1 }}>Just now</div>
              </div>
            </div>
...
function PhoneMockup() {
  ...
  return (
    <div style={{ position: 'relative', width: 185 }}>
      <div style={{ position: 'absolute', left: -4, top: 78,  width: 4, height: 28, background: 'linear-gradient(180deg,#d0d0d2,#aaaaac)', borderRadius: '3px 0 0 3px', zIndex: 5 }} />
      <div style={{ position: 'absolute', left: -4, top: 115, width: 4, height: 37, background: 'linear-gradient(180deg,#d0d0d2,#aaaaac)', borderRadius: '3px 0 0 3px', zIndex: 5 }} />
      <div style={{ position: 'absolute', left: -4, top: 161, width: 4, height: 37, background: 'linear-gradient(180deg,#d0d0d2,#aaaaac)', borderRadius: '3px 0 0 3px', zIndex: 5 }} />
      <div style={{ position: 'absolute', right: -4, top: 130, width: 4, height: 56, background: 'linear-gradient(180deg,#d0d0d2,#aaaaac)', borderRadius: '0 3px 3px 0', zIndex: 5 }} />
      <div style={{
        background: 'linear-gradient(155deg, #f2f2f4 0%, #c6c6ca 28%, #dadadd 55%, #f5f5f7 82%, #c0c0c4 100%)',
        borderRadius: 41, padding: 4,
        boxShadow: '-20px 40px 80px rgba(0,0,0,0.65), 3px 3px 0 rgba(255,255,255,0.2) inset, -2px -2px 0 rgba(0,0,0,0.12) inset',
      }}>
        <div style={{ background: '#0A0C10', borderRadius: 38, overflow: 'hidden', position: 'relative' }}>
          <div style={{ position: 'absolute', top: 12, left: '50%', transform: 'translateX(-50%)', width: 76, height: 21, background: '#000', borderRadius: 999, zIndex: 20 }} />
          <div style={{ padding: '14px 16px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative', zIndex: 10 }}>
            <span style={{ fontSize: 9, fontWeight: 700, color: 'rgba(255,255,255,0.85)' }}>9:41</span>
            <div style={{ display: 'flex', gap: 3, alignItems: 'flex-end' }}>
              {[4, 7, 10, 14].map((h, i) => (
                <div key={i} style={{ width: 3, height: h, borderRadius: 1, background: i < 3 ? 'rgba(255,255,255,0.8)' : 'rgba(255,255,255,0.22)' }} />
              ))}
              <div style={{ width: 18, height: 9, borderRadius: 2, border: '1.5px solid rgba(255,255,255,0.45)', position: 'relative', marginLeft: 3 }}>
                <div style={{ position: 'absolute', inset: 1.5, borderRadius: 1, background: '#9AD872', width: '72%' }} />
                <div style={{ position: 'absolute', right: -3, top: '50%', transform: 'translateY(-50%)', width: 2, height: 5, background: 'rgba(255,255,255,0.3)', borderRadius: 1 }} />
              </div>
            </div>
          </div>
          <div style={{ padding: '7px 14px 9px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 700, fontSize: 13, color: '#fff' }}>POS</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 3, padding: '2px 7px', borderRadius: 999, background: 'rgba(70,132,50,0.15)', border: '1px solid rgba(70,132,50,0.25)' }}>
                <div style={{ width: 4, height: 4, borderRadius: '50%', background: '#9AD872' }} />
                <span style={{ fontSize: 8, color: '#9AD872', fontWeight: 600 }}>LIVE</span>
              </div>
              <div style={{ width: 24, height: 24, borderRadius: '50%', background: '#468432', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#fff' }}>F</div>
            </div>
          </div>
          <div style={{ margin: '9px 12px 7px', background: '#181A20', borderRadius: 9, padding: '7px 10px', display: 'flex', alignItems: 'center', gap: 6, border: '1px solid rgba(255,255,255,0.07)' }}>
            <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.25)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
            <span style={{ fontSize: 8.5, color: 'rgba(255,255,255,0.22)' }}>Search or scan barcode</span>
          </div>
          <div style={{ display: 'flex', gap: 4, padding: '0 12px', marginBottom: 9, overflow: 'hidden' }}>
            {['All', 'Food & Bev', 'Drinks', 'Beef'].map((cat, i) => (
              <div key={cat} style={{ padding: '3px 9px', borderRadius: 999, whiteSpace: 'nowrap', background: i === 0 ? '#9AD872' : '#181A20', color: i === 0 ? '#0A0C10' : 'rgba(255,255,255,0.38)', fontSize: 8, fontWeight: i === 0 ? 700 : 400, border: i === 0 ? 'none' : '1px solid rgba(255,255,255,0.08)' }}>
                {cat}
              </div>
            ))}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, padding: '0 12px 10px' }}>
            {mobileProducts.map((p, i) => (
              <div key={i} style={{ background: '#181A20', borderRadius: 9, padding: '8px 8px 7px', border: '1px solid rgba(255,255,255,0.07)' }}>
                <div style={{ background: '#23252D', borderRadius: 5, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 6 }}>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/>
                    <polyline points="3.27 6.96 12 12.01 20.73 5.95"/><line x1="12" y1="22.08" x2="12" y2="12"/>
                  </svg>
                </div>
                <div style={{ fontSize: 8.5, color: 'rgba(255,255,255,0.72)', lineHeight: 1.3, marginBottom: 3 }}>{p.name}</div>
                <div style={{ fontSize: 9, color: '#9AD872', fontWeight: 700 }}>{p.price}</div>
              </div>
            ))}
          </div>
          <div style={{ margin: '0 12px 12px', background: '#468432', borderRadius: 11, padding: '9px 13px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 6px 20px rgba(70,132,50,0.4)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
              <div style={{ width: 19, height: 19, borderRadius: 999, background: 'rgba(255,255,255,0.22)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: '#fff' }}>3</div>
              <span style={{ fontSize: 9.5, color: 'rgba(255,255,255,0.85)', fontWeight: 500 }}>items in cart</span>
            </div>
            <span style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, fontSize: 9.5, color: '#fff' }}>View Cart →</span>
          </div>
        </div>
      </div>
    </div>
